...
| Git | Create and checkout feature/chore branches, commit, push | Commit or push directly to `{{DEFAULT_BRANCH}}` |
...
3a. **Branch Creation Gate (P-011, NON-NEGOTIABLE)**: Before claiming (the first workspace mutation), ensure a feature branch is active:
    ...
    - If on `{{DEFAULT_BRANCH}}` (the default branch):
...
11. **Branch retention (NON-NEGOTIABLE)**: Remain on the feature or chore branch until the
    PR is successfully merged. Do NOT checkout `{{DEFAULT_BRANCH}}` or any other branch
...
2. Confirm the merge SHA is present in the default branch history (separate sequential steps — do not chain):
   ...
   - Exit code 0: merge commit confirmed in `origin/{{DEFAULT_BRANCH}}` history. Proceed.
   - Non-zero: halt with `MERGE_NOT_CONFIRMED: merge SHA {merge_sha} is not yet in origin/{{DEFAULT_BRANCH}} history. Wait for the push to propagate.`
...
compound refresh, compact-context). These commits MUST NOT land directly on `{{DEFAULT_BRANCH}}`.
...
2. **Create a post-merge closure branch** from `{{DEFAULT_BRANCH}}` (run as separate sequential steps):
   `git checkout {{DEFAULT_BRANCH}}`
   `git pull`
...
3. **All subsequent Step 6 work happens on this branch.** Every commit in steps 6.1–6.10
   targets `post-merge/{feature_slug}`, not `{{DEFAULT_BRANCH}}`.
...
work. Committing directly to `{{DEFAULT_BRANCH}}` bypasses code review and violates the
...
* **Stay on the feature branch** from Step 1 through Step 5 merge approval. Never checkout
  `{{DEFAULT_BRANCH}}` or another branch while the feature PR is open.
* **Create a `post-merge/{feature_slug}` branch** for all Step 6 closure work. Never commit
  post-merge closure artifacts directly to `{{DEFAULT_BRANCH}}`.
