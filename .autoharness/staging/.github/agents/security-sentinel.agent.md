...
* **Injection Risk Assessment** — SQL injection, XSS, command injection, template injection, LDAP injection patterns in `{{SOURCE_GLOB}}`
...
Language-specific detection uses `{{SECURITY_SCAN_PATTERNS}}`.
...
1. Scan source files matching `{{SOURCE_GLOB}}` for unvalidated external inputs
2. Apply injection detection patterns from `{{SECURITY_SCAN_PATTERNS}}`
...
Score the workspace against OWASP Top 10 categories using `{{SECURITY_SCAN_PATTERNS}}`:
...
Save the audit report to `{{DOCS_SECURITY}}/YYYY-MM-DD-HH-MM-security-sentinel.md` where `YYYY-MM-DD` is today's date and `HH-MM` is the current time (24-hour). The time component prevents clobbering earlier runs on the same day.
...
* May create/write the audit report under `{{DOCS_SECURITY}}/` — this is the only permitted write operation
