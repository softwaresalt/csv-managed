...
* Local tools required by this skill must be available in PATH, including the configured quality gates `{{QUALITY_GATE_1}}`–`{{QUALITY_GATE_4}}`; if formatting fixes are applied during remediation, `{{FORMAT_FIX_COMMAND}}` must also be available
...
| `poll_interval` | int | `{{POLL_INTERVAL}}` | Seconds between CI status polls during the push-and-poll step. |
| `max_wait` | int | `{{MAX_WAIT}}` | Maximum total seconds to wait for CI to reach a terminal state. |
...
1. `cargo fmt --all -- --check` → if fails, run `{{FORMAT_FIX_COMMAND}}`
2. `cargo clippy -- -D warnings` → fix violations
...
|---|---|---|
| Run auto-fix command | Formatting diff exists and auto-fix is configured (`{{FORMAT_FIX_COMMAND}}`) | Auto-fix introduces semantic changes or breaks tests |
