...
2. Prefer table-driven or parameterized tests when the task describes
   multiple scenarios.
3. Create matching production stubs with {{UNIMPLEMENTED_MARKER}} bodies
...
* **Unit harnesses**: colocated with the production code in the
  appropriate {{SOURCE_DIR}} subdirectory
...
the expected failure marker ({{UNIMPLEMENTED_MARKER}}).
