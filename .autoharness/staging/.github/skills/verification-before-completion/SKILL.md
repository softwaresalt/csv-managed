---
# Source: references/ai-skills/skills/verification-before-completion/SKILL.md
# License: MIT
name: verification-before-completion
description: 'Enforces fresh verification evidence before any completion claim. Use when about to claim "tests pass", "bug fixed", "done", "ready to merge", or handing off work.'
---
...
| Backend handler / endpoint | Send a request to the endpoint, verify response shape and status code, hit at least one error path (invalid input, missing auth) |
| CLI tool | Run the binary with real inputs, check stdout, stderr, and exit code |
| Infra / IaC | Run a dry-run or plan command; review the diff before applying |
...
| Refactoring (no behavior change) | Full test suite passes unchanged. Public API surface diff shows no breakage |
...
- **Orphan op**: delete/update/get a nonexistent ID — does it return the expected error, or throw an internal error?
...
- **Configuration/infra** -- verify syntax (e.g., JSON/YAML validators, dry-run commands). If no validator exists, read the file and confirm it matches the intended change.
...
Generated by autoharness | Template: community/skills/verification-before-completion/SKILL.md.tmpl
