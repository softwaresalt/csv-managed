---
# Source: references/ai-skills/skills/debugging/SKILL.md
# License: MIT
name: debugging
description: 'Systematic root-cause debugging with verification. Use for errors, stack traces, broken tests, flaky tests, regressions, or anything not working as expected.'
---
...
- Property-based test
- Manual reproduction steps documented in a scratch file
...
Run once to gather evidence showing WHERE it breaks, then investigate that specific component. Log BEFORE the dangerous operation, not after it fails. Include context: working directory, environment variables, stack trace.

**Pre-existing failure proof:** Before claiming a test failure is "not related to our changes," prove it. Run the test suite on clean state to confirm the failure exists on the base branch. Pre-existing without receipts is a lazy claim.
...
**6. Fix and verify** -- create a failing test FIRST, then fix. Run the test. Confirm the original reproduction case passes. No completion claims without fresh verification evidence.
...
**Six failure categories** to consider: code logic, data/state, environment/config, concurrency/timing, external dependency, infrastructure.

**Evidence strength scale**: direct reproduction (strongest), log correlation, code path analysis, circumstantial (weakest).

## Intermittent Issues

For race conditions, deadlocks, resource exhaustion, and timing-dependent bugs:
- **Shared mutable state**: look for global variables, singletons, class-level caches accessed from multiple threads
- **Check-then-act**: find patterns where a condition is checked and then acted on without atomicity
- **Circular lock acquisition**: trace lock ordering across call paths
- **Connection pool exhaustion**: monitor pool metrics under load

## Defense-in-Depth Validation

After fixing, validate at every layer -- not just where the bug appeared. Apply the four-layer pattern:
1. **Entry layer**: input validation, request parsing
2. **Business logic layer**: domain rules, state transitions
3. **Environment layer**: configuration, dependencies, file system
4. **Instrumentation layer**: logging, metrics, alerting
...
For non-trivial production bugs, write a lightweight postmortem covering: timeline, root cause, impact, fix, and prevention measures.

Generated by autoharness | Template: community/skills/debugging/SKILL.md.tmpl
