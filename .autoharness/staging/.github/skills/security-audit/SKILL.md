...
| Variable | Purpose |
|---|---|
| `{{AGENTIC_CONFIG_GLOB}}` | Glob patterns for agentic config files (e.g., `.github/**`, `.vscode/**`) |
| `{{SOURCE_GLOB}}` | Application source file patterns (e.g., `src/**/*.{{FILE_EXT}}`) |
| `Rust` | Primary language for pattern selection |
| `{{DOCS_SECURITY}}` | Output directory for persisted reports (default: `docs/security`) |
...
1. Enumerate agentic config surfaces matching `{{AGENTIC_CONFIG_GLOB}}`
2. Enumerate application source files matching `{{SOURCE_GLOB}}`
...
Rules from `{{SECURITY_CONFIG_RULES}}`:
...
Scan source files matching `{{SOURCE_GLOB}}` (or the specified path) using `{{SECURITY_OWASP_PATTERNS}}`:
...
Save the report to `{{DOCS_SECURITY}}/YYYY-MM-DD-HH-MM-security-audit.md` where `YYYY-MM-DD` is today's date and `HH-MM` is the current time (24-hour). The time component prevents clobbering earlier runs on the same day when the skill is invoked multiple times (different scopes or modes).

If `{{DOCS_SECURITY}}/` does not exist, create it before writing.
...
* The report is persisted to `{{DOCS_SECURITY}}/` before the audit is marked complete
