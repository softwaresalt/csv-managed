---
# Source: references/ai-skills/skills/meta-prompting/SKILL.md
# License: MIT
name: meta-prompting
description: 'Structured reasoning modifiers (/think, /verify, /adversarial, /edge, /confidence, /assumptions, etc.) to stress-test decisions, surface assumptions, or enumerate edge cases. Use when validating an important design, architecture decision, or ambiguous plan before committing.'
---
...
**`/verify-think`** | `/check` -- Three phases: (1) **Answer** direct response, (2) **Challenge** 3 ways it could be wrong, (3) **Verify** investigate each, update if needed. Mark final as `VERIFIED ANSWER:` or `REVISED ANSWER:`.
...
Patterns auto-trigger in the following contexts (the agent notes which pattern was applied):

| Context | Pattern |
|---|---|
| Security, validation, parsing | `/edge` |
| Architecture decisions, critical choices | `/verify-think` |
| Architecture decisions at scale | `/flip` |
| Architecture reviews, ambiguous requirements | `/assumptions` |
| Code review requests | `/analyze` combo |
| Trade/position discussions | `/trade` combo |
...
Generated by autoharness | Template: community/skills/meta-prompting/SKILL.md.tmpl
