...
' -f owner="softwaresalt" -f repo="csv-managed" -F pr=<pr_number>
```
...
' -f owner="softwaresalt" -f repo="csv-managed" -F pr=<pr_number> -f threadCursor=""
```
...
```bash
gh api repos/softwaresalt/csv-managed/commits/<head_sha>/check-runs \
...
```bash
gh api repos/softwaresalt/csv-managed/check-runs/<check_run_id>/annotations \
