...
Prefer explicit, typed error handling with context at the boundary where failures matter.

### Naming

Follow the repository's established naming rules for files, types, and symbols.

### Documentation

Document public surfaces and non-obvious decisions close to the code that owns them.

### Testing

* TDD required: write tests first, verify they fail, then implement
* Test tiers in `tests/` directory:
Primary test command: cargo test
