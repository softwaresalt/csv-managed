...
#![forbid(unsafe_code)] unless justified with // SAFETY: comment. All clippy warnings are hard errors (-D warnings). All errors must be propagated via Result with contextual messages.
...
directory structure (Unit tests in src/ (inline #[cfg(test)]), integration tests in tests/) MUST be maintained. All tests MUST
