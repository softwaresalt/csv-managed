...
| `* Check .github/ for overly permissive allow-lists
* Check .vscode/settings.json for terminal auto-approve patterns
* Check for hardcoded credentials or API keys
* Check for unpinned secrets in CI workflows
* Check for debug endpoints or verbose logging of sensitive data` | Per-environment config rule table |
| `* unsafe blocks with unchecked deserialization
* Command injection via std::process::Command with user input
* Path traversal via unchecked user-supplied paths
* Unchecked integer overflow in index arithmetic
* Missing input validation at system boundaries` | Language-specific OWASP detection patterns |
...
Rules from `* Check .github/ for overly permissive allow-lists
* Check .vscode/settings.json for terminal auto-approve patterns
* Check for hardcoded credentials or API keys
* Check for unpinned secrets in CI workflows
* Check for debug endpoints or verbose logging of sensitive data`:
...
Scan source files matching `src/**` (or the specified path) using `* unsafe blocks with unchecked deserialization
* Command injection via std::process::Command with user input
* Path traversal via unchecked user-supplied paths
* Unchecked integer overflow in index arithmetic
