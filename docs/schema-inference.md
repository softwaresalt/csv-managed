...
* `--mapping` applies snake_case renames with collision prevention. Renames do not influence datatype inference.
...
for each column:
  init TypeCandidate
for each row until sample limit:
  decode cells
  skip empty
  if placeholder -> record summary; continue
  candidate.update(cell)
decide datatype via TypeCandidate::decide()
apply overrides
...
write schema (infer only unless --preview)
```
