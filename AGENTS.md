...
Single-repository Rust CLI project. The Stage agent manages planning and triage; the Ship agent manages implementation, review, and delivery. Both agents share the same workspace and follow the constitution and workflow policies.
...
#![forbid(unsafe_code)] unless justified with // SAFETY: comment. All clippy warnings are hard errors (-D warnings).
All errors must be propagated via Result with contextual messages.
